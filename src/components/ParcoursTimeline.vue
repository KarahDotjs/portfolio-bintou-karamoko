    ...
    <div class="container">
      ...
      <div class="parcours-header reveal" ref="header">
        ...
        <h2 class="section-title">
          Plusieurs vies,<br>une seule <span>logique</span>
        </h2>
      </div>
      ...
      <div class="parcours-outer">
        ...
        <!-- PÉRIODE 1 -->
        <div class="tl-col reveal reveal-d1" ref="col1">
          <div class="tl-period-badge badge-p1">2016 – 2018</div>
          ...
          <div class="tl-steps">
            <div class="tl-step" v-for="(step, i) in periode1" :key="i" :style="{ transitionDelay: `${0.3 + i * 0.1}s` }" :class="{ visible: lineDrawn }">
              <div class="tl-step-date">{{ step.date }}</div>
              <div class="tl-step-text" v-html="step.text"></div>
            </div>
          </div>
        </div>

        <!-- FLÈCHE 1 -->
        <div class="tl-arrow reveal reveal-d2">
          <div class="tl-arrow-line"></div>
          <div class="tl-arrow-head"></div>
          <div class="tl-arrow-label">pivot</div>
        </div>

        <!-- PÉRIODE 2 -->
        <div class="tl-col reveal reveal-d2" ref="col2">
          <div class="tl-period-badge badge-p2">2019 – 2024</div>
          ...
          <div class="tl-steps">
            <div class="tl-step amber" v-for="(step, i) in periode2" :key="i" :style="{ transitionDelay: `${0.5 + i * 0.1}s` }" :class="{ visible: lineDrawn }">
              <div class="tl-step-date amber">{{ step.date }}</div>
              <div class="tl-step-text" v-html="step.text"></div>
            </div>
          </div>
        </div>
        ...
        <!-- FLÈCHE 2 -->
        <div class="tl-arrow reveal reveal-d3">
          <div class="tl-arrow-line"></div>
          <div class="tl-arrow-head"></div>
          <div class="tl-arrow-label">retour</div>
        </div>

        <!-- PÉRIODE 3 -->
        <div class="tl-col reveal reveal-d3" ref="col3">
          <div class="tl-period-badge badge-p3">2026 – 2028</div>
          ...
          <div class="tl-steps">
            <div class="tl-step teal" v-for="(step, i) in periode3" :key="i" :style="{ transitionDelay: `${0.7 + i * 0.1}s` }" :class="{ visible: lineDrawn }">
              <div class="tl-step-date teal">{{ step.date }}</div>
              <div class="tl-step-text" v-html="step.text"></div>
            </div>
          </div>
        </div>
      ...
      <!-- BLOC FINAL -->
      <div class="tl-final reveal reveal-d4" :class="{ visible: lineDrawn }">
        <div>
          <div class="tl-final-label">Objectif 2028 +</div>
          <div class="tl-final-text">Cloud Security Engineer</div>
        </div>
        <div class="tl-final-badge">En route ›</div>
      </div>
...
onMounted(() => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach(e => {
        if (e.isIntersecting) {
          setTimeout(() => { lineDrawn.value = true }, 200)
          observer.disconnect()
        }
      })
    },
  ...
  if (col1.value) observer.observe(col1.value)
})
...
.tl-connector.drawn { width: 100%; }
.tl-connector.p1 { background: linear-gradient(90deg, rgba(0,43,92,0.5), rgba(0,43,92,0.08)); }
.tl-connector.p2 { background: linear-gradient(90deg, rgba(196,122,0,0.5), rgba(196,122,0,0.08)); }
.tl-connector.p3 { background: linear-gradient(90deg, #0052a5, rgba(0,200,224,0.3)); }
...
.tl-step {
  ...
  border-radius: 10px;
  border: 1px solid rgba(0, 82, 165, 0.07);
  opacity: 0;
...
.tl-step.amber {
  border-color: rgba(196, 122, 0, 0.1);
  background: rgba(255, 248, 235, 0.75);
}
.tl-step.teal {
  border-color: rgba(0, 82, 165, 0.1);
...
.tl-step-date.amber { color: #8a5700; }
...
.tl-step-text :deep(strong) {
  color: var(--navy);
  font-weight: 600;
}

/* ── Flèches entre périodes ── */
.tl-arrow {
  display: flex;
  flex-direction: column;
  align-items: center;
...
@media (max-width: 900px) {
  ...
  .tl-arrow-head {
    ...
    border-left: 7px solid rgba(0, 119, 255, 0.3);
  }
  .tl-arrow-label {
